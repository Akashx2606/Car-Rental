    ...
    bg-[#F1F5F9] text-center'>
        ...
        <form onSubmit={handleSearch} className='flex flex-col md:flex-row items-start md:items-center
            ...
            <div className='flex flex-col items-start gap-2'>
                ...
                <input onChange={e=>setPickupDate(e.target.value)} value={pickupDate} type='date' id="pickup-date" min={new Date().toISOString().split('T')[0]}
                className='text-sm text-grey-500' required/>
            </div>

            <div className='flex flex-col items-start gap-2'>
                <label htmlFor='return-date'>Return Date</label>
                <input onChange={e=>setReturnDate(e.target.value)} value={returnDate} type='date' id="return-date" min={new Date().toISOString().split('T')[0]}
                className='text-sm text-grey-500' required/>
            </div>
            </div>
            <button className='flex items-center justify-center gap-1 px-9 py-3 max-sm:mt-4
            bg-[#2563EB] hover:bg-[#1F58D8] text-white rounded-full cursor-pointer'> 
